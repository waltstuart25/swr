...
def test_bengen1():
    ...
    assert z.iloc[0]['before_spend'] * 1.03 == pytest.approx(107.5, 0.000001), "first before spend does not match Bengen"
    assert z.iloc[0]['end_port'] * 1.03 == pytest.approx(103.38, 0.000001), "first ending port does not match Bengen"
    assert z.iloc[-1]['before_spend'] * 1.03 == pytest.approx(129.79, 0.001), "last before spend does not match Bengen"
    assert z.iloc[-1]['end_port'] * 1.03 == pytest.approx(125.67, 0.001), "last ending port does not match Bengen"
...
def test_bengen2():
    ...
    download_df = load_returns()
    return_df = download_df.iloc[:, [0, 3, 16]] # wjs cpi column was 12?
    ...
    assert z[0]['trial'].iloc[0]['end_port'] == pytest.approx(120.959978, 0.000001), "bad value: cohort 0 year 0 end port"
    assert z[0]['trial'].iloc[-1]['spend'] == 4.0, "bad value: cohort 0 final year spend"
    assert z[0]['trial'].iloc[-1]['end_port'] == pytest.approx(184.802029, 0.000001), "bad value: cohort 0 final year end port"
    assert z[-1]['trial'].iloc[0]['spend'] == 4.0, "bad value: last cohort year 0 spend"
    assert z[-1]['trial'].iloc[0]['end_port'] == pytest.approx(106.171324, 0.000001), "bad value: cohort 0 year 0 end port"
    assert z[-1]['trial'].iloc[-1]['spend'] == 4.0, "bad value: last cohort final year spend"
    assert z[-1]['trial'].iloc[-1]['end_port'] == pytest.approx(291.380494, 0.000001), "bad value: cohort 0 final year end port"
